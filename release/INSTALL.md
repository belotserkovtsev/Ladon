...
mkdir -p /opt/split-engine/state /etc/split-engine
cd /tmp
curl -L -O "https://github.com/belotserkovtsev/split-engine/releases/download/${TAG}/split-engine-linux-${ARCH}.tar.gz"
tar xzf split-engine-linux-${ARCH}.tar.gz

# Распаковался каталог split-engine-linux-${ARCH}-${TAG}/
cd split-engine-linux-${ARCH}-${TAG}

install -m 0755 split-engine             /opt/split-engine/split-engine
install -m 0644 split-engine.service     /etc/systemd/system/
install -m 0644 manual-allow.txt.example /etc/split-engine/manual-allow.txt
install -m 0644 manual-deny.txt.example  /etc/split-engine/manual-deny.txt
```
...
```bash
# Создать схему БД
/opt/split-engine/split-engine \
  -db /opt/split-engine/state/engine.db \
  init-db

# Включить сервис
systemctl daemon-reload
systemctl enable --now split-engine
...
sqlite3 /opt/split-engine/state/engine.db \
...
sqlite3 -column /opt/split-engine/state/engine.db \
...
echo "myblocked.com" >> /etc/split-engine/manual-allow.txt

# Добавить в never-touch list
echo "mybank.ru" >> /etc/split-engine/manual-deny.txt

# Перечитать (engine читает только при старте)
systemctl restart split-engine
```

## 7. Удаление

```bash
systemctl disable --now split-engine
rm /etc/systemd/system/split-engine.service
rm -rf /opt/split-engine /etc/split-engine
ipset destroy prod
