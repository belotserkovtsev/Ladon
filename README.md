...
**От первого DNS-запроса клиента до правила в kernel ipset — в среднем полсекунды.** Следующий TCP-пакет уходит уже правильным маршрутом. В постоянный список попадает только то, что подтвердилось ≥50 раз за 24 часа — моргания провайдера фильтруются на уровне `hot` и до постоянной памяти не доезжают.

| Метрика | Значение |
|---|---|
| **Реакция на новый блок** — от DNS-запроса до правила в ядре | **0.3 – 1.1 с** (в среднем 0.5 с) |
| **Пропускная способность** — доменов через полный цикл «запрос → probe → решение → ipset» | **~65/с** (на 2 CPU) |
| **Накладные расходы пайплайна** — сверх чистой сетевой задержки probe | **~50 мс** |
| **RSS процесса** — при ~500 доменов в БД, несколько десятков в hot/cache | **~20 МБ** |

### Почему быстро и точно

- **Нет split-brain с клиентом.** Probe бьёт по IP из `dns_cache` — ровно тех, что dnsmasq уже отдал клиенту. На geo-routed CDN движок и клиент иначе резолвились бы в разные адреса, и ipset промахивался бы.
- **Параллельные TCP-dials.** На все IP домена одновременно, побеждает первый успешный; остальные отменяются через shared-context. Худший случай по времени становится `max(timeout)` вместо `sum(timeout)`.
- **Event-driven ipset.** Probe сигнализирует syncer через буферный канал сразу после вердикта, без ожидания тикера. Safety-reconcile раз в 30 с ловит потерянные сигналы — ни один Hot не остаётся без правила в ядре.
- **eTLD+1 агрегация.** Если ≥2 поддомена одного корня подтверждены — в ipset добавляются IP-ы всей семьи. Meta и прочие CDN, которые раздают новые UUID-поддомены ежеминутно, не успевают утечь через direct.
