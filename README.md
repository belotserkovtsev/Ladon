...
Обычный VPN ставит выбор без правильного ответа: **«всё через туннель»** ломает рос-сервисы которые требуют РФ-IP (банки, Госуслуги) и нагружает exit-канал, а **«всё напрямую»** не открывает заблокированное. Ladon держит середину: пир ходит напрямую по умолчанию, ладон в фоне смотрит DNS-трафик клиентов и сам переключает через туннель только то, что реально не работает direct'ом.
...
**Три источника правды для ipset:**

- **Auto-discovery** — ладон тестирует каждый новый домен из DNS-трафика. Если TCP/TLS-handshake фейлится — IP отправляется в `ladon_engine` ipset, дальше iptables разворачивает соответствующий трафик клиента в туннель. Стабильные блоки запоминаются на постоянной основе, временные — забываются через сутки.

- **Extensions / manual-allow** — для сервисов которые блокируют твой регион **на своей стороне** (ChatGPT, Spotify — TCP/TLS у них работает, но в HTTP-ответе «not available in your country»). Probe такое не отличит от рабочего, поэтому решение принимает оператор. Ладон пишет их в конфиг dnsmasq, и dnsmasq нативно кладёт IP в отдельный `ladon_manual` ipset при резолве.

- **manual-deny** — список доменов которые НЕ должны идти через туннель никогда: внутренние LAN-сервисы, рос-сервисы которые ломаются с иностранного exit'а (Госуслуги, банки).

**Routing — твоя зона.** Ладон только наполняет ipset'ы. Куда направлять трафик когда destination IP в ipset (через какой fwmark, какую таблицу маршрутизации, на какой tunnel-интерфейс) — решает оператор iptables-правилами. Чёткое разделение: ладон отвечает за **что** тоннелировать, оператор — за **как**.

<details>
<summary><b>🔌 Глубокая схема пайплайна — для тех кто хочет копать</b></summary>
