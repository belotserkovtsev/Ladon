...
Автоматика закрывает блоки **изнутри** — ту часть, где DPI режет соединение на пути от клиента к ресурсу. Probe видит TCP RST или TLS handshake failure и заворачивает домен в туннель. Но есть второй класс блокировок, который автоматика принципиально **не может определить**:

- **`manual-allow.txt`** — домены, которые блокируют твой регион **со своей стороны** (геофенс на IP). С точки зрения probe всё работает: TCP коннект устанавливается, TLS handshake проходит — но сервис в ответ на HTTP-запрос показывает «not available in your country», или редиректит, или просто отказывает в логине. ChatGPT, Anthropic, OpenAI, Spotify, многие SaaS — типичные примеры. Probe их не отличит от ресурсов, которые действительно работают, поэтому решение принимает оператор: «знаю, что снаружи блокирует — гнать через туннель безусловно». Также сюда же — содержимое-уровневые блоки (HTTP body filtering, SNI-спуфинг), и принудительный туннель по приватным соображениям.
...
Для типовых наборов гео-блокированных сервисов используй extensions (см. ниже) — это просто curated `manual-allow` списки, которые шипаются с ладоном.
