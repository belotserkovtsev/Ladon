...
- **В туннель.** Адреса домена попадают в список в ядре (`ipset`), а твои правила
  ...
  годности. Насколько постоянный, зависит от того, включена ли перепроверка
  (см. ниже): без неё решение держится, пока его не снимут руками.
...
stateDiagram-v2
    ...
    Hot --> covered: набралось 10+ поддоменов сайта
    cache --> Ignore: ручная чистка
    cache --> [*]: перепроверка сняла решение
    covered --> Ignore: сайт перестал подтверждаться
    Ignore: Ignore, напрямую
    Hot: Hot, в туннеле на 24ч
    cache: cache, в туннеле навсегда
    covered: covered, в туннеле со всем сайтом
```

Короткие имена дальше по тексту: `Hot`, `cache` и `covered` означают «в туннеле»,
`Ignore` означает «напрямую».
...
Технически Ladon подписан на DNS-сервер и берёт оттуда имена, которые запросили
клиенты: у `dnsmasq` читает лог запросов, у `unbound` слушает сокет. Нет
запроса, нет и кандидата на пробу.

Чьи это запросы, зависит от того, где Ladon стоит. На шлюзе он видит весь дом,
на отдельной машине с резолвером в контейнере — только её собственные обращения.
Метод от этого не меняется, меняется охват.
...
| `tls_intercept` | TLS | рукопожатие прошло, но сертификат подписан не тем, кому мы верим | путь | в туннель |
...
Три кода стоит пояснить отдельно:
...
- **`tls_intercept`.** Рукопожатие прошло целиком, соединение установлено, но
  сертификат на той стороне не выводится ни к одному корню, которому мы верим.
  Так выглядит заглушка провайдера на 443: чтобы говорить по TLS, ей приходится
  подписать сертификат самой себе, а её подпись ничего не значит. Для пробы,
  которая смотрит только на достижимость, это выглядело бы полностью рабочим
  сайтом, поэтому здесь единственное место, где Ladon вообще заглядывает в
  сертификат, и смотрит он туда не ради безопасности, а ради этого признака.

  Несовпадение имени в сертификате в сигнал **не** входит. Здоровые хосты
  сплошь и рядом отвечают на именах, которых их сертификат не покрывает
  (легаси-адреса S3, Apple Push), и если считать это подменой, они разом
  уезжают в туннель ни за что. Считается только неизвестный подписант.
...
## Решение живёт, пока подтверждается

Накопление отвечает за то, как домен попадает в постоянный список. Остаётся
обратный вопрос: как он оттуда уходит.

Сеть под решением меняется. Блокировку снимают, домен переезжает, а сайт,
который открывался годами, начинают резать. Вердикт, принятый однажды и никем
больше не проверенный, со временем начинает врать в обе стороны: держит в
туннеле то, что давно открывается, и пропускает мимо то, что уже нет.

Поэтому `cache` и `ignore` не окончательные. Ладон время от времени
берёт домен, которого давно не касался, и пробует его заново. Если результат
противоречит текущему состоянию несколько раз подряд, решение снимается, и
домен возвращается к обычным пробам, будто его видят впервые. Дальше он
классифицируется с нуля: всё ещё заблокированный снова наберёт себе `cache`,
отпущенный осядет в `ignore`.

Важна именно серия, а не единичное расхождение. Одна удачная проба к
заблокированному домену случается регулярно (DPI сработал нестабильно, попали
на другой узел CDN), и снимать по ней постоянное решение значило бы дёргать
список на каждом шорохе.

Перепроверка выключена по умолчанию и включается в
[конфигурации](configuration.md#перепроверка-принятых-решений), там же
описаны интервал, размер серии и то, сколько это занимает по времени.

## Сайт целиком: семья доменов

Крупные сервисы живут не на одном домене, а на десятках поддоменов: сам сайт,
вход, картинки, CDN. Блокируют их обычно заодно, всей семьёй. Поэтому, когда у
одного сайта накапливается достаточно подтверждённых поддоменов в постоянном
списке (по умолчанию 10), Ladon перестаёт проверять остальные по одному и
заворачивает в туннель весь сайт сразу.

Новые поддомены такого сайта попадают в туннель без отдельной пробы, в состоянии
`covered`. Это и точнее (соседние поддомены почти наверняка режут так же), и
дешевле: не нужно гонять отдельную пробу на каждый новый адрес CDN, которых у
большого сервиса бывают сотни. Если со временем сайт перестаёт набирать
подтверждения, его поддомены возвращаются к обычным пробам.
...
Все четыре отвечают на вопрос, как домен попадает в решение. За то, как он из
решения выходит, отвечает отдельная и тоже необязательная перепроверка: она
возвращает в общую очередь домены, чьё состояние перестало подтверждаться.
