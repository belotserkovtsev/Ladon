...
| `youtube` | allow | YouTube: сайт, видеопоток, превью, API плеера |
...
`youtube` — тот случай, когда пробам домен не отдать вовсе. Прямой путь к нему
не рвётся с ошибкой, а молча уходит в таймаут, и адреса при этом непрерывно
меняются: движок обновляет свой список раз в полминуты, а клиент успевает уйти
по свежему адресу напрямую раньше. Домены из подборки dnsmasq заносит в список
во время резолва, до первого пакета клиента, поэтому гонки не возникает.
