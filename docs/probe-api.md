...
`probe.mode: remote`. Фиксирован — имплементация на стороне потребителя может
использовать любой язык и любую внутреннюю механику (raw TCP, HTTP GET,
...
## Decision path в ладоне

1. Ладон шлёт запрос. Если транспорт упал (сеть, таймаут, non-200) — результат
   эквивалентен `tcp_ok=false && tls_ok=false`, reason префиксован `remote:`.
2. Если проб-сервер вернул `tcp_ok=true && tls_ok=true` → `decision.Classify`
   возвращает `Ignore` (домен доступен, тоннель не нужен).
3. Иначе → `Hot` с 24-часовым TTL. Дальше — обычный путь: hot_entries →
   scorer → cache_entries → ipset.
